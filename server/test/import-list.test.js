...
describe("settings of one deck (#137, migration 014)", () => {
  ...
  it("starts a list at 10 new cards a day and Kaishi at her overall limit, with every way on", async () => {
    ...
    assert.deepEqual(byName.Kaishi.settings, { hiddenModes: [], newPerDay: 20, maxPerDay: null, extraNew: 0, extraNewDay: null, flipFront: "word", reverse: false });
    assert.deepEqual(byName.long.settings, { hiddenModes: [], newPerDay: 10, maxPerDay: null, extraNew: 0, extraNewDay: null, flipFront: "meaning", reverse: false });
  ...
  it("stores the ways of practising per deck, and never all five hidden", async () => {
    ...
    assert.deepEqual(res.json().settings, { hiddenModes: ["choose", "type"], newPerDay: 10, maxPerDay: null, extraNew: 0, extraNewDay: null, flipFront: "meaning", reverse: false });
    const { decks } = await json("/api/decks");
